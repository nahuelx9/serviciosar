...
public interface ServicioDao extends JpaRepository<Servicio, Integer> {
    ...
    @Modifying
    @Query(value = "UPDATE servicio SET nombre=?,celular=?,whatsapp=?,email=?,precio=?,precio_descripcion=?,horario=?,descripcion=?,id_usuario=? where id_servicio=?",nativeQuery= true)
   public void actualizarServicio(String nombre, String celular,boolean whatsaap,String email,String precio,String precio_descripcion, String horario, String descripcion,int id_usuario,int id_servicio);
